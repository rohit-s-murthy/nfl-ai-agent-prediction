import { Game, TeamStats, WeatherConditions } from '../models/game';
import { getTeamStats, getTeamByAbbreviation, NFLTeam } from '../data/nflData';
import { getHeadToHeadRecord, getRecentPerformance, getSeasonRecord, HeadToHeadRecord } from '../data/gameHistory';
...
export function generateComprehensivePrompt(game: Game): string {
...
export function generateQuickPrompt(homeTeam: string, awayTeam: string, week: number): string {
    const homeStats = getTeamStats(homeTeam);
    const awayStats = getTeamStats(awayTeam);
    const headToHead = getHeadToHeadRecord(homeTeam, awayTeam);
...
function buildMatchupContext(game: Game): MatchupContext {
    ...
    const homeRecentPerformance = getRecentPerformance(game.homeTeam);
    const awayRecentPerformance = getRecentPerformance(game.awayTeam);
...
function formatRecentForm(recentGames: any[], teamAbbr: string): string {
...
export function generateWeeklyPrompts(week: number, season: number = 2025): string[] {
